...
// ============================================================
// CircleModel.tsx — 3D Circular-Cell Structural Framework (Radial Matrix)
// Repeated connected 3D circular cells in continuous matrix with omnidirectional deformation,
...
import { THRESHOLD_CIRCLE_N } from "@/lib/simulation/constants";

interface CircCellData {
  id: number;
...
function generateCircleCells(cellCount: number): CircCellData[] {
  const cells: CircCellData[] = [];
  let id = 0;
  const count = clamp(Math.round(cellCount), 3, 6);
  const pitch = 3.2 / count;

  for (let r = -count / 2; r < count / 2; r++) {
    for (let c = -count / 2; c < count / 2; c++) {
      const x = c * pitch + pitch / 2;
      const z = r * pitch + pitch / 2;
      const pos = new THREE.Vector3(x, 0, z);
      const dir = pos.clone().normalize();
      dir.x += (Math.sin(id * 1.6) - 0.5) * 0.4;
      dir.y += 0.4 + Math.abs(Math.cos(id * 2.4)) * 0.6;
      dir.z += (Math.cos(id * 1.6) - 0.5) * 0.4;
      dir.normalize();
      ...
      cells.push({
        ...
        rotAxis: new THREE.Vector3(Math.sin(id * 2.5), Math.cos(id * 1.8), Math.sin(id * 3.2)).normalize(),
        rotSpeed: 1.7 + (id % 3) * 1.1,
      });
    }
  }
...
export function CircleModel() {
  ...
  const cells = useMemo(() => generateCircleCells(cellCount), [cellCount]);
  const groupRef = useRef<THREE.Group>(null);
  const burstRef = useRef<THREE.Mesh>(null);

  const isFailed = loadN >= THRESHOLD_CIRCLE_N;
  ...
  const count = clamp(Math.round(cellCount), 3, 6);
  const pitch = 3.2 / count;
  const outerS = pitch / 2;
  const wtRatio = clamp(wallThicknessMm / cellSizeMm, 0.08, 0.45);
  const holeR = Math.max(outerS * (1 - wtRatio), outerS * 0.2);
  const height = 1.8;
    ...
    <group>
      ...
      >
        {cells.map((c) => {
          ...
          const demand = clamp(spatialFactor * (loadN / THRESHOLD_CIRCLE_N), 0, 1);
      ...
      {isFailed && (
        <group position={[0, 4.0, 0]}>
          <Text position={[0, 0, 0]} fontSize={0.34} color="#E05252" anchorX="center" anchorY="middle">
            ⚠ STRUCTURAL FAILURE: RADIAL MATRIX FRACTURED
          </Text>
          <Text position={[0, -0.4, 0]} fontSize={0.22} color="#62748A" anchorX="center" anchorY="middle">
            {`(Circle threshold: ${THRESHOLD_CIRCLE_N} N exceeded)`}
          </Text>
        </group>
      )}
    </group>
